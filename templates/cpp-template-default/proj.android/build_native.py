...
def do_build(cocos_root, ndk_root, app_android_root,ndk_build_param,sdk_root,android_platform,build_mode,jobs):
    ...
    if ndk_build_param == None:
        command = '%s -j%d -C %s NDK_DEBUG=%d' % (ndk_path, jobs, app_android_root, build_mode=='debug')
    else:
        command = '%s -j%d -C %s NDK_DEBUG=%d %s' % (ndk_path, jobs, app_android_root, build_mode=='debug', ' '.join(str(e) for e in ndk_build_param))
...
def copy_resources(app_android_root):
    ...
    # copy resources
    os.mkdir(assets_dir)
    resources_dir = os.path.join(app_android_root, "../Resources_android")
    if os.path.isdir(resources_dir):
        copy_files(resources_dir, assets_dir)
        
    animations_dir = os.path.join(assets_dir, "animations") 
    os.mkdir(animations_dir)
    resources_dir = os.path.join(app_android_root, "../DevResources/animations")  
    if os.path.isdir(resources_dir):
        copy_files(resources_dir, animations_dir)

def build(ndk_build_param,android_platform,build_mode,jobs):
    ...
    do_build(cocos_root, ndk_root, app_android_root,ndk_build_param,sdk_root,android_platform,build_mode,jobs)
...
if __name__ == '__main__':
    ...
    parser.add_option("-j", "--jobs", dest="jobs", type="int", help='number of jobs')
    (opts, args) = parser.parse_args()
    
    build(opts.ndk_build_param,opts.android_platform,opts.build_mode,opts.jobs)
