      ...
      foreach(_dir ${include_dir_candidates})
        if(EXISTS ${_dir})
          ...
          foreach(_inc_name ${_${_lib}_inc})
            ...
            set(_inc_tmp)
            if(EXISTS ${_dir}/${_inc_name})
              set(_inc_tmp ${_dir})
            else()
              foreach(_suffix ${_suffixes})
                if(EXISTS ${_dir}/${_suffix}/${_inc_name})
                  set(_inc_tmp ${_dir}/${_suffix})
                  break()
                endif()
              endforeach()
            endif()
          ...
          foreach(_lib_name ${_${_lib}_libs})
            ...
            foreach(_prefix ${CMAKE_FIND_LIBRARY_PREFIXES})
              foreach(_suffix ${CMAKE_FIND_LIBRARY_SUFFIXES})
                if(EXISTS ${_dir}/${_prefix}${_lib_name}${_suffix})
                  set(_lib_tmp ${_dir}/${_prefix}${_lib_name}${_suffix})
                  break()
                endif()
              endforeach()
            endforeach()
