var LibraryWebSocket = {
    WebSocket_init: function(thiz, address)
    {
        ...
        Module['websocket']._callbacks[thiz] = {};

        var serverAddress = Pointer_stringify(address);
        var socket = new WebSocket(serverAddress);
        socket.binaryType = "arraybuffer";
    
        Module['websocket']._socketMap[thiz] = socket;

        socket.onopen = function()
        {
            Module['websocket'].emit('open', thiz, [null, 0]);
        };
        ...
        socket.onmessage = function(event)
        {
            if (typeof event.data === "string")
            {
                ...
                Module['websocket'].emit('message', thiz, [msg, event.data.length]);
            ...
            else if (event.data instanceof ArrayBuffer)
            {
                ...
                Module['websocket'].emit('message', thiz, [buffer, event.data.byteLength]);
                _free(buffer);
            }
        };
        ...
        socket.onclose = function(event)
        {
            ...
            Module['websocket'].emit('close', thiz, [event.code, msg, reason.length]);
            Runtime.stackRestore(sp);
        };
        ...
        socket.onerror = function(error)
        {
            ...
            Module['websocket'].emit('error', thiz, []);
    ...
    {
        if(Module['websocket'].socketMap[thiz] != undefined)
        {
			var socket = Module['websocket'].socketMap[thiz];
			socket.onopen = function(){ };
			socket.onmessage = function(event) { };
			socket.onclose = function(event){ };
			socket.onerror = function(error) { };
